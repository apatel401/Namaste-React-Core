...
function RestaurantMenu() {
  ...
  // const getC = () => {
  //    const output = restaurant?.menu?.items && [...new Map(Object.values(restaurant?.menu?.items).map(v => [v.category, v])).values()]
  //   setCatogories(getCategories)
  // }
  return !restaurant ? (
    <CardShimmer />
  ) : (
      ...
      <div className="w-1/2 mx-auto">
        ...
        {Object.values(restaurant?.menu?.items).map((item) => (
          <div key={item.id} className="flex justify-between p-8 border-b-2">
            ...
            {item?.cloudinaryImageId ? (<><img
                src={IMG_CDN_URL + item?.cloudinaryImageId}
                className="w-full max-w-full"
              /><button className="rounded-md bg-pinky p-2 my-4 mx-auto flex justify-center text-base">Add</button></>) : <button className="rounded-md bg-pinky p-2 my-4 mx-auto flex justify-center text-base">Add</button>}
            </div>
          </div>
        ))}
      </div>
